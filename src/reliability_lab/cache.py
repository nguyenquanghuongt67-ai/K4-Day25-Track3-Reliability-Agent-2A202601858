...
class ResponseCache:
    ...
    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        ...
        if _is_uncacheable(query):
            return
        entry = CacheEntry(key=query, value=value, created_at=time.time(), metadata=metadata or {})
        self._entries.append(entry)

    @staticmethod
    def similarity(a: str, b: str) -> float:
        """Compute semantic similarity between two strings using character 3-grams and word tokens."""
        if a == b:
            return 1.0
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        for key in self._redis.scan_iter(f"{self.prefix}*"):
            ...
            cached_resp = self._redis.hget(key, "response")
            if cached_query and cached_resp:
                score = ResponseCache.similarity(query, cached_query)
                if score > best_score:
                    best_score = score
                    best_value = cached_resp
