    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            ...
            if to_state == "open":
                ...
                recovery_times.append((float(ts) - open_ts) * 1000.0)
                open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    for _ in range(config.load_test.requests):
        ...
        res = gateway.complete(q)
        metrics.total_requests += 1
        metrics.estimated_cost += res.estimated_cost

        if res.cache_hit:
            metrics.cache_hits += 1
            metrics.estimated_cost_saved += 0.001

        if res.route == "fallback":
            metrics.fallback_successes += 1
            metrics.successful_requests += 1
        elif res.route == "static_fallback":
        ...
        if res.latency_ms > 0:
            metrics.latencies_ms.append(res.latency_ms)

    open_count = 0
    for breaker in gateway.breakers.values():
        open_count += sum(1 for entry in breaker.transition_log if entry.get("to") == "open")
